...
/* =========================
   Prompts (Work-specific only)
   ========================= */

const TEXT_SYSTEM_PROMPT = `You are an event extractor for professional/work schedules.

Input is normalized text lines (some are flattened table rows joined with " | ").

Extract ONLY dated work items with concise names.

Ignore in names: locations/rooms, dial-in/URLs, email addresses, long agenda details, RSVP notes.
  ...
  "events": [
    {
      ...
      "event_tag": "meeting|1:1|standup|review|retrospective|planning|interview|call|demo|presentation|workshop|training|conference|offsite|onsite|travel|deadline|deliverable|launch|holiday|pto|other" | null
    }
  ]
}
...
- Prefer compact patterns like "Product Review", "Sprint Planning", "QBR", "Client Demo", "Hiring Interview", "Team Standup".
- Preserve useful identifiers like "Q3", "v1.2", "RFP #104".
- Use current year if missing (${new Date().getFullYear()}).
- Accept dates like YYYY-MM-DD, MM/DD, M/D, "Oct 5", "October 5".
- Times: "12 pm"→"12:00"; ranges use start time; "EOD"/"COB"→"17:00"; "noon"→"12:00"; "midnight"→"00:00".
- If deadline/due and no time → "17:00"; otherwise if no time, event_time = null.
...
const VISION_SYSTEM_PROMPT = `You are an event extractor reading business calendars as images (use built-in OCR).
...
- For calendar grids/tables, read month/year from headers and carry forward until a new header appears.
...
Name noise to ignore: room/building, dial-in/URLs, emails, map links, RSVP verbiage, agenda details.

Combine vs split:
- Single item with multiple qualifiers ("Launch v1.2 + Press Briefing") → split into separate events.
- Same item repeated with times ("Standup 9:00 / 9:30") → create separate events per start time.
...
- "EOD"/"COB"→"17:00".
- If deadline/due and no time → "17:00"; else if no time → null.
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: "system", content: TEXT_SYSTEM_PROMPT },
        {
          role: "user",
          content:
            `Batch ${batchIndex + 1}/${totalBatches}. Work mode: normalize & ignore noise (rooms, dial-ins, URLs, emails, long agenda text).\n` +
            `Extract ONLY dates + actionable work items (meetings, interviews, reviews, deadlines, launches). Multiple items per date allowed.\n` +
            `---BEGIN---\n${batchText}\n---END---`,
        },
      ],
      temperature: 0.0,
  ...
  const userContent: Array<any> = [
    {
      type: "text",
      text:
        "Work mode: Extract events WITH DATES from these calendar images. Every event MUST have event_date in YYYY-MM-DD. " +
        "Resolve dates via headers/columns/cells; if still unknown, omit. Normalize US-style dates and carry forward month/year from headings. Split distinct items (e.g., 'Launch + Press Briefing').",
    },
  ];
...
/* =========================
   Text Input Service (Work-specific)
   ========================= */

const TEXTBOX_SYSTEM_PROMPT = `You are a work-focused calendar parser. Extract professional events from natural language and return JSON.

Current date for reference: ${new Date().toISOString().split('T')[0]}

Naming & Formatting (critical):
- SHORT, professional Title-Case "event_name" (≤ 50 chars).
- Prefer concise work patterns: "Client Meeting", "Team Standup", "Sprint Planning", "QBR", "Hiring Interview", "Release Launch", "Design Review".
- No dates/times/pronouns in event_name; no dial-in/URLs/locations/emails.

Tag guidance:
- Use one of: meeting, 1:1, standup, review, retrospective, planning, interview, call, demo, presentation, workshop, training, conference, offsite, onsite, travel, deadline, deliverable, launch, holiday, pto, other. If unclear → null.

Other Rules:
- If year missing, use current year (${new Date().getFullYear()}).
- Dates: YYYY-MM-DD
- Times: HH:MM (24-hour)
- "EOD"/"COB" → "17:00"; "noon"→"12:00"; "midnight"→"00:00"; ranges use start time.
- If deadline/due and no time → "17:00"; else if time missing → null.
